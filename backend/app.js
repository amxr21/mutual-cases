 ...
 *
 * Builds and returns the configured app WITHOUT binding a port — so tests can
 * import it (and drive it with supertest) without starting a server or touching
 * the real DB. server.js imports this and calls listen().
 */
...
/**
 * Build the app. Rate limiting is opt-out so tests aren't throttled across many
 * requests (pass { rateLimit: false }); production passes nothing (enabled).
 */
function createApp({ rateLimit: enableRateLimit = true } = {}) {
    ...
    // origin, so the browser never renders HTML from it — drop the document-
    // oriented CSP/CORP defaults, keep nosniff/frameguard/HSTS/referrer-policy.
    app.use(
        helmet({
    ...
    // Abuse protection. Broad per-IP limiter plus a stricter one on auth.
    ...
    // 404 for anything unmatched, then the centralized error handler. Order
    // matters: these must be registered AFTER all routes.
